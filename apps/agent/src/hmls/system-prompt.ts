...
4. Call \`create_order\` (with \`customerInfo: { phone, address }\` filled in) to save the draft. For each service, also pass \`jobSlug\` = the \`slug\` from its \`lookup_labor_time\` match, and pass each exact \`recommendedPart\` from \`lookup_parts_price\` in \`referenceParts\` — this attaches tools/difficulty and the saved catalog reference for the shop's tech prep (internal only, never shown to the customer). Never invent or alter a part number.
